...
def encrypt(key, msg):
    return strxor(key, msg).encode('hex')
...
cArr = [c1,c2,c3,c4,c5,c6,c7,c8,c9,c10]

target = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

def ms(s, c):
    return "".join([c for i in range(len(s))])

#returns msg xord-ed bitwise with c
def testChar(msg, c):
    return encrypt(msg, ms(msg,c))

def testAllChars(msg):
    print("".join([c+": "+testChar(msg, c)+'\n' for c in chars]))

def testAllSubs(cipher, testStr):
    for i in range(0,len(cipher)-len(testStr)):
        n = encrypt(cipher[i:], ms(cipher[i:], testStr))
        print(n[0:5])
        if n[0:5] == "00000":
            print(n)

def freq(s):
    r = [0]*128 #bins for all letters (and capitals)
    for i in range(len(s)/2):
        bits = s[2*i:2*i+2]
        index = int(bits, 16) #i.e. A -> 65; a -> 97
        r[index] += 1
    return r
...
def getAllFreq(arr):
    ...
    for i in range(len(arr)-1):
        for j in range(i+1,len(arr)):
            n = freq(encrypt(arr[i], arr[j]))
            r = [i+j for i,j in zip(r,n)]
    return r
